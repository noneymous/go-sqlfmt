...
func write(buf *bytes.Buffer, INDENT, NEWLINE, WHITESPACE string, token, previousToken lexer.Token, indent int, hasManyClauses bool) {
	switch {
	case token.IsNewlineKeyword():
	...
	default:
		if hasManyClauses {
			...
			if previousToken.IsNewlineKeyword() || previousToken.Type == lexer.AND || previousToken.Type == lexer.OR {
...
func separate(rs []lexer.Reindenter, WHITESPACE string) []interface{} {
	var (
		result           []interface{}
		skipRange, count int
	)
...
// TODO: more elegant
func processPunctuation(rs []lexer.Reindenter, WHITESPACE string) ([]lexer.Reindenter, error) {
	var (
...
func extractSurroundingArea(rs []lexer.Reindenter, WHITESPACE string) (string, int, error) {
	var (
		countOfStart int
		countOfEnd   int
		result       string
